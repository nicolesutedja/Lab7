describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Access the shadowRoot and find the button inside it
    const button = await firstItem.evaluateHandle(el => el.shadowRoot.querySelector('button'));

    // Click the button
    await button.click();

    // Get the innerText of the button
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // Grab all <product-item> elements
    const allItems = await page.$$('product-item');

    // Loop through every product item and click its button
    for (let i = 1; i < allItems.length; i++) {
      const button = await allItems[i].evaluateHandle(el => el.shadowRoot.querySelector('button'));
      await button.click();
    }

    // Grab the #cart-count element and get its innerText
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    // Grab the cart item from localStorage
    const cart = await page.evaluate(() => {
      return localStorage.getItem('cart');
    });

    // Expect it to be the array of all 20 item IDs
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    // Click the button on every item to remove from cart
    for (let i = 0; i < allItems.length; i++) {
      const button = await allItems[i].evaluateHandle(el => el.shadowRoot.querySelector('button'));
      await button.click();
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // Check every button says "Add to Cart"
    let allAdd = true;
    for (let i = 0; i < allItems.length; i++) {
      const button = await allItems[i].evaluateHandle(el => el.shadowRoot.querySelector('button'));
      const innerText = await button.getProperty('innerText');
